...
│   ├── dc_blocker.h         # DC blocking filter
...
The generated `src/eq_coeffs.h` is ready to `#include` in your ESP‑IDF project.
The DSP code in `src/enhancer.c` + `src/biquad_q28.h` + `src/envelope.h` +
`src/dc_blocker.h` compiles directly under ESP‑IDF — no changes needed.
