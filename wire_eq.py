#!/usr/bin/env python3
"""
wire_eq.py — End‑to‑end: measure speaker → build LADSPA plugin → wire to output.

Usage:
    # Setup: measure, fit, build, install, wire
    python3 wire_eq.py setup <speaker_name>
    python3 wire_eq.py setup -m response.wav -n noise.wav -t target.wav

    # Teardown: remove the pipeline, restore defaults
    python3 wire_eq.py teardown

    # Just build the plugin (no wiring):
    python3 wire_eq.py build <speaker_name>

setup does:
  1. Runs the full EQ pipeline (eqgen.run_pipeline)
  2. Fits IIR biquads (eq_fit.fit_eq_curve)
  3. Quantizes to Q4.28 (quantize.quantize_biquads_q28)
  4. Generates src/eq_coeffs.h
  5. Builds eqgen_ladspa.so + enhancer.so
  6. Installs LADSPA plugin to ~/.ladspa/
  7. Creates a null sink (eqgen_sink) and sets it as default
  8. Starts an audio bridge piping through enhancer_ffi → hardware output

Environment:
    EQGEN_LATENCY_MS    Bridge latency in ms (default 20)
"""
...
def get_pw_rate():
    """Read the PipeWire graph sample rate from pactl."""
    import re
...
from eq_fit import fit_eq_curve, cascade_response_db
from quantize import BiquadQ28, quantize_biquads_q28, q28_to_float
from eqgen import run_pipeline
...
def _pw_link(out_node, out_port, in_node, in_port):
    """Create a pw-link. Assumes port names like <node>:<port_name>."""
...
def get_module_index(name_filter):
    """Find a loaded module index by partial name match."""
...
def generate_eq_header(freqs, coeffs_q28_list, bands, cfg):
    """Write src/eq_coeffs.h with baked-in coefficients."""
    coeffs_flat = []
    for bq in coeffs_q28_list:
        coeffs_flat.extend([bq.b0, bq.b1, bq.b2, bq.a1, bq.a2])

    lines = []
    lines.append("// Auto-generated by wire_eq.py — do not edit by hand.")
    lines.append(f"// {len(bands)} biquads, Q4.28 format")
    lines.append("// Regenerate: python3 wire_eq.py setup ...")
    lines.append("")
    ...
    for i, bq in enumerate(coeffs_q28_list):
        band = bands[i]
        lines.append(
    ...
    header = SRC_DIR / "eq_coeffs.h"
    header.write_text("\n".join(lines) + "\n")
    print(f"  Wrote {header}  ({len(bands)} biquads)")


# ──────────────────────────────────────────────────────────────────────
#  pipeline: measurement → biquads
# ──────────────────────────────────────────────────────────────────────
...
def run_full_pipeline(speaker_name=None, meas_paths=None, noise_path=None,
    ...
    """Run the complete pipeline and return quantized biquads + config.

    target_fs: if given, design biquads at this rate instead of the
               measurement WAV rate (e.g. pass PipeWire's 48000).
               The EQ curve (freq, gain) is rate-independent.
    """

    # Resolve measurement files
    if speaker_name:
    ...
    q28_floats = []
    for bq in bq_q28:
        from eq_fit import BiquadCoeffs
    ...
    for label, lo, hi in [
        ("Bass ≤250Hz", 0, 250), ("Mid 250-2k", 250, 2000), ("Treble >2k", 2000, 99999),
    ]:
    ...
    cfg = {
        ...
        "h3_amp": h3,
        "release_secs": 0.2,
        "limiter_release_secs": 0.049,
        "fs": float(design_fs),
    }

    # Flatten coefficients for JSON
    coeffs_q28_flat = []
    for bq in bq_q28:
        coeffs_q28_flat.extend([bq.b0, bq.b1, bq.b2, bq.a1, bq.a2])

    return eq_freqs, bq_q28, fit.bands, cfg, coeffs_q28_flat
...
def setup_wiring(cfg, coeffs_q28_flat):
    """Create null sink, launch bridge, link explicitly, set as default."""

    STATE_DIR.mkdir(parents=True, exist_ok=True)
    state = {
        **cfg,
        "coeffs_q28": coeffs_q28_flat,
        "n_biquads": len(coeffs_q28_flat) // 5,
    }
    ...
    print(f"── PW rate: {pw_rate} Hz  |  DSP rate: {cfg['fs']:.0f} Hz  (PW resamples)")

    # ── create null sink ──
    print(f"\n── Creating eqgen_sink (null sink)...")
    r = _pactl("load-module", "module-null-sink",
               "sink_name=eqgen_sink", "sink_properties=device.description=EQGen")
    if r.returncode != 0:
    ...
    # ── create explicit links (port names are input_1/input_2 etc) ──
    print(f"\n── Creating links...")
    ...
    if not links_ok:
        print("  ⚠️  Links may not be correct. Check with: pw-link -l | grep eqgen")
    else:
    ...
    # ── set eqgen sink as default (LAST — after links are established) ──
    print(f"\n── Setting eqgen_sink as default...")
    set_default_sink("eqgen_sink")
    print(f"  Default sink: {get_default_sink()}")

    print(f"\n  ✅ Audio now flows: app → eqgen_sink → native DSP filter → {prev_sink}")
    print(f"  Run './wire_eq.sh teardown' to restore.")


def teardown_wiring():
    """Restore previous state from state file.  Robust against corrupt state."""
    state = {}
    ...
    # ── kill bridge (pid + pgid) ──
    pid = state.get("bridge_pid")
    if pid:
        print(f"Killing bridge (PID {pid})...")
        # Try process group first (bridge started with start_new_session)
        try:
    ...
    # ── sweep any leftover eqgen processes ──
    import glob
    for proc in glob.glob("/proc/*/comm"):
        try:
            with open(proc) as f:
                cmd = f.read().strip()
        except OSError:
            continue
        if cmd in ("pw-cat", "eqgen_bridge", "bash"):
            # Check cmdline for eqgen references
            cmdline_path = proc.replace("/comm", "/cmdline")
            try:
                ...
                continue
            if "eqgen" in cmdline:
                pid_str = os.path.basename(os.path.dirname(proc))
                try:
                    os.kill(int(pid_str), signal.SIGKILL)
                    print(f"  Killed leftover {cmd} (PID {pid_str})")
                except OSError:
                    pass
    ...
    if mod_id:
        ...
        # Fallback: unload all null-sink modules (may be too broad)
        r = _pactl("list", "modules", "short")
        for line in r.stdout.splitlines():
...
def main():
    ...
    p_setup.add_argument("-n", "--noise", default=None,
                         help="Noise WAV file (for spectral subtraction)")
    ...
    p_setup.add_argument("--release", type=float, default=0.2,
                         help="Envelope release seconds [0.2]")
    ...
    p_build.add_argument("speaker", help="Speaker name (under measurements/)")
    ...
    if args.command == "setup":
        ...
        generate_eq_header(eq_freqs, bq_q28, bands, cfg)
