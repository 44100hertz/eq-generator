...
def analyze_chebyshev_mix(
    ...
    from harmonic_bass import design_butter_lp, design_butter_hp, biquad_tick
    ...
    # ── 6. HP filter effect ────────────────────────────────────────────
    # Compute HP gain at f, 2f, 3f
    hp_state_f = np.zeros(4)
    hp_state_2f = np.zeros(4)
    hp_state_3f = np.zeros(4)

    # Use long sine to measure steady-state gain
    test_n = int(0.5 * fs)
    tt = np.arange(test_n) / fs
    for i in range(test_n):
        sf = np.sin(2 * np.pi * freq * tt[i])
        s2f = np.sin(2 * np.pi * 2 * freq * tt[i])
        s3f = np.sin(2 * np.pi * 3 * freq * tt[i])
        _, hp_state_f = biquad_tick(sf, hp_coeffs, hp_state_f)
        _, hp_state_2f = biquad_tick(s2f, hp_coeffs, hp_state_2f)
        _, hp_state_3f = biquad_tick(s3f, hp_coeffs, hp_state_3f)
    ...
    # ── 7. Summary ─────────────────────────────────────────────────────
    # The total harmonic output at frequency f after HP:
    #   Principal: HP_filtered(original) at f  (from the main signal path)
    #   Leak:      T₃ fundamental leakage at f  (from the harmonic path, also HP filtered)
    #
    # Original signal at f, after input HP: A_eff * hp_gain_f
    # Leakage at f, after HP: t3_leak_corrected * hp_gain_f
    ...
    # The "overtone": the leakage is at the SAME frequency as the original,
    # but if it's large and inverted, it causes partial cancellation of the
    # fundamental, making the harmonics (at 2f, 3f) RELATIVELY louder.
    # This sounds like the fundamental got quieter / the overtones got louder.
...
def butterworth_hp_mag(f: float, fc: float) -> float:
    """2nd-order Butterworth HP magnitude response."""
    w = f / fc
...
def param_sweep():
    ...
    for f in freqs:
        # LP gain at this frequency
        omega = np.tan(np.pi * f / 44100.0)
        # Actually just use the magnitude formula
